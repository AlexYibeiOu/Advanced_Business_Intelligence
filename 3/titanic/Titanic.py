#Titanic is a Kaggle project.
#Two submissions was made by 17th, April 2020
#Using Linear Regression in the 1st and 2nd submission
...
print(combined.iloc[:889]['Age'].isnull().sum())
print(combined.iloc[889:]['Age'].isnull().sum())

#let's get the median age based on people's gender, Pclass and Title
grouped_train = combined.iloc[:889].groupby(['Sex','Pclass','Title'])
...
for c in combined.iloc[:889]['Cabin']:
    try:
        train_cabin.add(c[0])
    except:
        train_cabin.add('M')
...
X_train, X_test, y_train, y_test = train_test_split(df_im_input, 
df_im_output, train_size=0.8, test_size=0.2, random_state=0)
